      ...
      <div className="container-fluid d-flex align-items-center justify-content-between py-2 px-4">
        <div className="egov-header-logo">
          <Link to="/" className="text-decoration-none d-flex align-items-center">
            <span className="fw-bold text-primary fs-5">{t('header.brand', '전자정부 기업 업무 템플릿')}</span>
          </Link>
        </div>
...
const ADMIN_MENU: { to: string; labelKey: string; fallback: string }[] = [
  ...
  // 기업 업무 템플릿 전용
  { to: '/admin/users', labelKey: 'nav.user', fallback: '업무사용자 관리' },
  { to: '/admin/codes', labelKey: 'nav.code', fallback: '공통코드 관리' },
  { to: '/admin/codes/classes', labelKey: 'nav.codeClass', fallback: '공통코드 분류' },
  { to: '/admin/menus', labelKey: 'nav.menuManage', fallback: '메뉴 관리' },
  { to: '/admin/programs', labelKey: 'nav.programManage', fallback: '프로그램 관리' },
  { to: '/admin/absences', labelKey: 'nav.absence', fallback: '사용자 부재' },
  { to: '/admin/login-policies', labelKey: 'nav.loginPolicy', fallback: '접속정책' },
  { to: '/admin/logs/login', labelKey: 'nav.loginLog', fallback: '로그인 로그' },
  { to: '/admin/logs/system', labelKey: 'nav.sysLog', fallback: '시스템 로그' },
]
  ...
  return (
    <footer className="egov-footer border-top mt-5 py-4">
      <div className="container-fluid px-4 d-flex flex-wrap gap-3 justify-content-between">
        <div>
          <p className="mb-1 fw-bold">{t('footer.title', '전자정부표준프레임워크 기업 업무')}</p>
