        ...
        <>
          {members.length === 0 ? (
            ...
            <div className="krds-table-wrap">
              <table className="tbl">
                <caption>{t('member.listCaption', '회원 목록 — 아이디, 이름, 이메일, 가입일, 상태')}</caption>
                <colgroup>
                  <col style={{ width: '18%' }} />
                  <col style={{ width: '18%' }} />
                  <col />
                ...
                <thead>
                  <tr>
                    ...
                    <th scope="col">{t('member.joinDate', '가입일')}</th>
                ...
                <tbody>
                  {members.map((member) => (
                    <tr key={member.uniqId}>
                      ...
                      <td>{member.mberNm ?? member.userNm ?? member.emplyrNm ?? '-'}</td>
                      <td className="text-start">{member.mberEmailAdres ?? member.emailAdres ?? '-'}</td>
                      <td>{member.sbscrbDe ?? '-'}</td>
