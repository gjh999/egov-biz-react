...
전자정부표준프레임워크(eGovFrame) 5.0 기반 **기업 업무 템플릿** 의 React 19 프론트엔드입니다.
...
| 저장소 | 역할 | 개발 포트 |
|---|---|---|
| [egov-biz-api](https://github.com/gjh999/egov-biz-api) | REST API 백엔드 | 28090 (`/api`) |
| **egov-biz-react** (이 저장소) | React 19 프론트 | 23000 |
| [egov-biz-vue](https://github.com/gjh999/egov-biz-vue) | Vue 3 프론트 (짝) | 23001 |
...
> 공유 파일: `client.ts` · `types.ts` · `auth.ts` · `biz.ts` (4개)
...
```bash
npm install
npm run dev     # http://localhost:23000
```

**백엔드가 먼저 떠 있어야 합니다.** [egov-biz-api](https://github.com/gjh999/egov-biz-api) 를 클론해
`mvn spring-boot:run` 으로 28090 포트에 띄우세요.

개발 서버는 `/api` 요청을 `http://localhost:28090` 로 프록시합니다(`vite.config.ts`).
...
| 화면 | 내용 |
|---|---|
| 업무사용자 관리 | 직원 계정 등록·수정·삭제 (일반회원과 **저장 테이블이 다름**) |
| 공통코드 분류 | 분류코드 관리 |
| 공통코드 | 코드 · 상세코드 관리 |
| 메뉴 관리 | 메뉴 구성 · 순서 · 상위메뉴 |
| 프로그램 관리 | 등록된 화면 목록, 메뉴와 연결 |
| 사용자 부재 | 부재 등록 · 기간 관리 |
| 접속정책 | 사용자별 로그인 허용 IP |
| 로그인 로그 | **조회 전용** |
| 시스템 로그 | **조회 전용** |

> **로그 화면에는 등록·수정·삭제가 없습니다.** 기록은 시스템이 자동으로 남기는 감사 추적 대상이라
> 화면에서 바꿀 수 있으면 안 되기 때문입니다.
...
저장값은 **단일 해시** `Base64(SHA-256(id ‖ password))` 이고, 해싱은 **서버**가 담당합니다.
...
### 3-5. 업무사용자와 일반회원은 저장 테이블이 다르다

그래서 관리 API 가 `/admin/users`(업무사용자)와 `/admin/members`(일반회원)로 나뉘어 있습니다.
**한쪽 API 로 다른 쪽 사용자를 조회하면 빈 결과가 나옵니다.**
...
```bash
docker build -t egov-biz-react:latest .
docker run --rm -p 23000:8080 -e BACKEND_URL=http://host.docker.internal:28090 egov-biz-react:latest
```

세 저장소를 한 번에 띄우려면 [egov-biz-api](https://github.com/gjh999/egov-biz-api) 의
...
│   └── biz.ts      게시판 · FAQ · Q&A · 설문 · 약관 · 회원 · 권한 · 업무사용자 · 코드 · 메뉴 · 로그
├── auth/           인증 상태 컨텍스트
├── i18n/           서버 메시지 번들 로딩 (GET /api/i18n/{ko|en})
├── components/     레이아웃 · 페이지네이션 · 로딩/오류 표시 · 관리자 CRUD 골격
├── pages/          화면
│   └── admin/      AdminPages.tsx (공통 17종) · BizAdminPages.tsx (기업 전용 9종)
└── test/           Vitest
```

**관리자 화면 26종은 하나의 골격(`AdminCrudPage`)을 공유합니다.** 도메인마다 다른 것은
컬럼 정의·필드 정의·API 호출 세 가지뿐이라, 화면을 추가할 때는 컴포넌트 하나만 더하면 됩니다.

**생성·수정·삭제 핸들러를 빼면 조회 전용 화면이 됩니다** — 로그인 로그·시스템 로그가 이 방식입니다.

---

## 8. 다국어

화면 문구의 원본은 **백엔드**([egov-biz-api](https://github.com/gjh999/egov-biz-api))의
`message-ui_{ko,en}.properties` 입니다. 앱 시작 시 `GET /api/i18n/{lang}` 으로 받아 씁니다.

문구는 `t('key', '기본값')` 형태로 씁니다. 두 번째 인자는 번들에 키가 없을 때 쓰이는 대비값입니다.

> 화면 문구 키 313 개 중 **310 개(99%)** 가 번들에 등록돼 있어 언어 전환이 실제로 동작합니다.
...
전자정부표준프레임워크 기업 업무 템플릿을 기반으로 합니다.
